...
int main(void)
{
    // List of possible cards first digits for Mastercard, Visa and American Express, used later for
    // validation
    int AE1 = 34;
    int AE2 = 37;
    int MC1 = 51;
    ...
    else if (count == 16)
    {
        ...
        if (temp16MC == MC1 || temp16MC == MC2 || temp16MC == MC3 || temp16MC == MC4 ||
            temp16MC == MC5)
        {
            brand = 3;
        }
    ...
    // if the number of digits is correct it now checks that luhn's Algorithm is respected,
    ...
    if (brand != 0)
    {
        ...
        while (card > 0)
        {
            ...
            // if the even number * 2 is 10 or bigger it takes the single digits and adds them the
            // rest of the odds
            if (tempx > 9)
            {
